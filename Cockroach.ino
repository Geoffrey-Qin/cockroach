...
if(millis () > hit_time){
 ...
 else{  if(dark == 0){        // move forward because the way is clear
...
if(val_right >= 120 && val_left >= 120){     //check if it is dark enough
  ...
  if(difference < -60){          // Check is the right side significantly darker or not, if it is run the following commands
  digitalWrite(4,LOW);
  digitalWrite(8,HIGH);
  turn_right();
  delay(1000);
